...
static NSString * const kCanniff_Philip_CodingYourAppJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
- (NSString *)_canniff_philip_codingyourappInjection
{
	if (pageToken==nil) {
	...
	[html appendString:kCanniff_Philip_CodingYourAppJavascript];
...
+ (NSString *)content:(NSString *)content withInjection:(NSString *)injection
{
	...
	// attempt to make well-formed HTML and inject in our Canniff_Philip_CodingYourApp bridge code
	   ...
	   baseURL:(NSURL*)baseURL
{
	...
	content = [[self class] content:content withInjection:[self _canniff_philip_codingyourappInjection]];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
	...
	if ([scheme hasPrefix:@"http"] || [scheme isEqualToString:@"ftp"]
		...
		if ([scheme isEqualToString:@"file"] || [scheme isEqualToString:@"app"]) {
			[NSURLProtocol setProperty:[self _canniff_philip_codingyourappInjection] forKey:kContentInjection inRequest:(NSMutableURLRequest *)request];
		}
		return YES;
	}
