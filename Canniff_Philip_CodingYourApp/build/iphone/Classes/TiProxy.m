...
//Rare exceptions to indicate a bug in the _canniff_philip_codingyourapp code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_canniff_philip_codingyourapp = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_canniff_philip_codingyourapp]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.canniff_philip_codingyourapp.module"
