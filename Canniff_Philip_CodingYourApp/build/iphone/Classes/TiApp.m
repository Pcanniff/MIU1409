...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kCanniff_Philip_CodingYourAppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kCanniff_Philip_CodingYourAppUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Canniff_Philip_CodingYourApp app using the node.js CLI
