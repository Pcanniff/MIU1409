...
NSString * Canniff_Philip_CodingYourApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Canniff_Philip_CodingYourAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_canniff_philip_codingyourapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_canniff_philip_codingyourapp gc];
}
...
{
	// create Canniff_Philip_CodingYourApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Canniff_Philip_CodingYourApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_canniff_philip_codingyourapp = [[Canniff_Philip_CodingYourAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_canniff_philip_codingyourapp];
	
	NSString *_canniff_philip_codingyourappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _canniff_philip_codingyourappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_canniff_philip_codingyourapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Canniff_Philip_CodingYourApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.canniff_philip_codingyourapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.canniff_philip_codingyourapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
